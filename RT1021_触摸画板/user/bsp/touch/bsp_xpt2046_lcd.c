 ...
 ******************************************************************************/
/* XPT2046引脚使用同样的PAD配置 */
#define TOUCH_PAD_CONFIG_DATA         (SRE_0_SLOW_SLEW_RATE| \
                                        DSE_6_R0_6| \
 ...
 * Prototypes
 ******************************************************************************/
static void XPT2046_IOMUXC_MUX_Config(void);
static void XPT2046_IOMUXC_PAD_Config(void);
static void XPT2046_ModeInit(void);
/**
...
static void XPT2046_IOMUXC_MUX_Config(void)
{
  /* MISO,IRQ引脚，需要使能SION以接收数据 */
  IOMUXC_SetPinMux(XPT2046_SPI_CS_IOMUXC, 0U);                                   
  IOMUXC_SetPinMux(XPT2046_SPI_CLK_IOMUXC, 0U);
  IOMUXC_SetPinMux(XPT2046_SPI_MOSI_IOMUXC, 0U);                                   
  IOMUXC_SetPinMux(XPT2046_SPI_MISO_IOMUXC, 1U);
  IOMUXC_SetPinMux(XPT2046_PENIRQ_IOMUXC, 1U);                                   
}
...
{
  /* SPI引脚 */
  IOMUXC_SetPinConfig(XPT2046_SPI_CS_IOMUXC, TOUCH_PAD_CONFIG_DATA);                               
  IOMUXC_SetPinConfig(XPT2046_SPI_CLK_IOMUXC, TOUCH_PAD_CONFIG_DATA);
  IOMUXC_SetPinConfig(XPT2046_SPI_MOSI_IOMUXC, TOUCH_PAD_CONFIG_DATA);                               
  IOMUXC_SetPinConfig(XPT2046_SPI_MISO_IOMUXC, TOUCH_PAD_CONFIG_DATA); 
  IOMUXC_SetPinConfig(XPT2046_PENIRQ_IOMUXC, TOUCH_PAD_CONFIG_DATA);                                	
}
/**
  * @brief  XPT2046 初始化函数
  * @param  无
...
{
  /* 定义gpio初始化配置结构体 */
  gpio_pin_config_t xpt2046spi_config;      

	XPT2046_IOMUXC_MUX_Config();
	XPT2046_IOMUXC_PAD_Config();	
	
   /** XPT2046 GPIO配置 **/       
  xpt2046spi_config.direction = kGPIO_DigitalOutput; //输出模式
  xpt2046spi_config.outputLogic =  1;                //默认高电平
  xpt2046spi_config.interruptMode = kGPIO_NoIntmode; //不使用中断
  
  /* 初始化 SPI GPIO. */
  GPIO_PinInit(XPT2046_SPI_CS_PORT, XPT2046_SPI_CS_PIN, &xpt2046spi_config);
  GPIO_PinInit(XPT2046_SPI_CLK_PORT, XPT2046_SPI_CLK_PIN, &xpt2046spi_config);
  GPIO_PinInit(XPT2046_SPI_MOSI_PORT, XPT2046_SPI_MOSI_PIN, &xpt2046spi_config);

   /** XPT2046 GPIO配置 **/       
  xpt2046spi_config.direction = kGPIO_DigitalInput; //输出模式
  xpt2046spi_config.interruptMode = kGPIO_NoIntmode; //不使用中断
	
  GPIO_PinInit(XPT2046_SPI_MISO_PORT, XPT2046_SPI_MISO_PIN, &xpt2046spi_config);	
  GPIO_PinInit(XPT2046_PENIRQ_GPIO_PORT, XPT2046_PENIRQ_GPIO_PIN, &xpt2046spi_config);		
...
//		PRINTF ( "usGap_x=%d,usGap_y=%d,", usGap_x,usGap_y);		
    if ( ( usGap_x > 20 ) || ( usGap_y > 20 ) ) goto Failure;       //可以通过修改这两个值的大小来调整精度    
  ...
  I2C_EEPROM_BufferRead(EEPROM_READ_ADDRESS_8_BIT,
	...
	{ 		
		...
		if(  para_flag == FLASH_TOUCH_PARA_FLAG_VALUE && forceCal == 1)
		{
			...
			I2C_EEPROM_BufferRead(EEPROM_READ_ADDRESS_8_BIT,
                           FLASH_TOUCH_PARA_ADDR,
                           (uint8_t *)&strXPT2046_TouchPara,
                           4*6*8);
		}
